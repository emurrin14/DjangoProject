...
from .models import Category, Product, ProductImage, Size, Color, ProductVariant, Sale, CustomUser, EmailVerificationToken, Cart, CartItem
...
class CartItemInline(admin.TabularInline):
    model = CartItem
    extra = 0
    readonly_fields = ('product', 'variant', 'quantity', 'subtotal_display', 'added_at')
...
class CartAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'session_key', 'total_items_display', 'total_price_display', 'created_at', 'updated_at')
    ...
    fieldsets = (
        ('Cart Information', {
            'fields': ('user', 'session_key', 'total_items_display', 'total_price_display')
        }),
        ('Timestamps', {
...
class CartItemAdmin(admin.ModelAdmin):
    list_display = ('id', 'cart', 'product', 'variant', 'quantity', 'subtotal_display', 'added_at')
    list_filter = ('added_at', 'cart__user')
    search_fields = ('product__title', 'cart__user__email', 'cart__session_key')
    readonly_fields = ('added_at', 'subtotal_display')
    date_hierarchy = 'added_at'
    
    fieldsets = (
        ('Item Information', {
            'fields': ('cart', 'product', 'variant', 'quantity', 'subtotal_display')
        }),
        ('Timestamps', {
            'fields': ('added_at',)
        }),
